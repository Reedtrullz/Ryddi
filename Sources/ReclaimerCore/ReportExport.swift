...
public final class ReportStore: @unchecked Sendable {
    ...
    @discardableResult
    public func save(projectDependencyPolicyDocument document: ProjectDependencyPolicyDocument) throws -> URL {
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        let url = root.appendingPathComponent("project-dependency-policy-\(document.id).json")
        try encoder.encode(document).write(to: url, options: .atomic)
        return url
    }
