...
- report-only Project Dependencies review for project-local node_modules, virtual environments, build folders, Pods, framework caches, Flutter/Android outputs, protected project files, optional local VCS status, saved per-project review policies, detected package managers, package.json script-risk previews, native rebuild command hints, and local audit history
