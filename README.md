...
Review First session groups expose an on-demand Session Review. It ranks local Codex and Hermes transcripts by size, reads Codex task titles from the local state index without scanning message contents, protects open sessions, and offers per-file Reveal, verified private Archive, or recoverable Finder Trash actions.
...
- Session Review selects nothing, bounds inventory at 5,000 files, and revalidates identity, modification state, allowed roots, and open handles before Archive or Trash.
