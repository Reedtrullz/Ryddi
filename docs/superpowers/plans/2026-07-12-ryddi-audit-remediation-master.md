...
Human QA remains separate from automated release proof. Ryddi-only About and exact-minimum-size sidebar states were captured, and privacy-bounded Finder, Dock, and app-switcher crops show the installed icon. A Finder capture containing unrelated filenames was removed from the public release, and a full-screen Dock capture containing unrelated desktop content was discarded locally. Subjective VoiceOver announcement quality is not claimed as reviewed. The installed release correctly reports degraded coverage because the local TCC database has no `kTCCServiceSystemPolicyAllFiles` grant for `com.reidar.ryddi`; regranting Full Disk Access is a user-controlled follow-up.
