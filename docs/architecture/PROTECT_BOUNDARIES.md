...
The app now links only `RyddiProtectCore` for a user-started Cloud Storage workspace. That workspace performs shallow local sync-root discovery, rejects symlink and overly broad roots, requires session-only confirmation, revalidates device/inode identity, and can build a bounded metadata-only local inventory. Traversal has entry, directory, depth, time, and retained-row limits; skips internal symlinks; distinguishes logical length from locally allocated blocks; and surfaces largest/oldest locally allocated files only as Finder review queues. It never opens file contents or hashes provider files, so it does not risk deliberate placeholder hydration for duplicate analysis. It does not link `RyddiProtectAuth`, connect a provider, issue a live network request, persist confirmation or cloud inventory, perform a real Keychain operation, inspect a dotenv value, upload a file, migrate a secret, influence cleanup classification, or authorize cleanup. ReclaimerCore and the CLI remain unaware of Protect provider types.
