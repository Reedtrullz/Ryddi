...
- Report-only Project Dependencies Review for project-local dependency/build artifact folders, protected project roots, local VCS status, saved per-project policies, detected package-manager/script/script-risk/workspace context, bounded/redacted package.json command previews, native rebuild command hints, and local audit history
