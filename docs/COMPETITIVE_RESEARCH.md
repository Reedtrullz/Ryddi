...
| Project dependency review | Expected for developer cleaners because project-local `node_modules`, virtualenvs, build outputs, and mobile artifacts can dominate disk usage | Exists/partial | `reclaimer projects` and app Project Dependencies now report project-local dependency/build artifact folders, ecosystem/kind/tool/script/VCS/policy summaries, protected project roots, saved per-project review/preserve/skip policies, detected package managers, accepted package.json scripts, and native rebuild command hints without modifying source, manifests, lockfiles, env files, credentials, IDE settings, dependencies, build outputs, or executing scripts. Future work should add richer monorepo/workspace awareness and project-script safety review. |
