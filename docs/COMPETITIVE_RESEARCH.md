...
| App uninstallers | AppCleaner, Pearcleaner, Nektony App Cleaner & Uninstaller, Hazel App Sweep | Remove apps plus related support files | App inventory, related files, leftovers, launch agents, containers, bulk uninstall, deselection, Trash | Ryddi now has review-only Apps & Leftovers plus selected-app preview and manual Finder evidence. Keep automatic leftover deletion, bulk uninstall, smart related-file selection, and app-bundle Trash execution out of the cleanup plan until ownership evidence is stronger. |
...
Ryddi should make permission state visible, avoid root/helper behavior in v1, use a per-user LaunchAgent for scheduled reports, and keep core filesystem cleanup manual rather than direct deletion; Homebrew is the narrow fresh-preview native-tool exception.
...
| Downloads and installer review | Expected in general Mac cleaners because old DMGs/PKGs/ZIPs are common visible bloat | Exists/partial | `reclaimer downloads` and app Downloads Review now report old downloads, installers, archives, app bundles, kind/workflow summaries, largest items, Finder review steps, app row actions, and local audit records without moving or deleting files. Future work should add saved Finder-style filters and optional user-confirmed Trash handoff where macOS allows it. |
| Browser cache review | Expected in broad cleaners, high-risk if profiles are blurred with cache | Exists/partial | `reclaimer browsers` and app Browser Cache Review now report browser cache roots, browser/cache-kind summaries, largest cache items, protected profile roots, advisory browser runtime status from local process-name matching, quit-first guidance, and local audit records without quitting browsers or modifying browser cache/profile state. Future work should add richer browser-specific cleanup guidance and optional notification flows. |
...
| Project dependency review | Expected for developer cleaners because project-local `node_modules`, virtualenvs, build outputs, and mobile artifacts can dominate disk usage | Exists/partial | `reclaimer projects` and app Project Dependencies now report project-local dependency/build artifact folders, ecosystem/kind/tool/script/script-risk/workspace/VCS/policy summaries, protected project roots, saved per-project review/preserve/skip policies, detected package managers and package names, workspace/monorepo evidence, bounded/redacted package.json command previews, script-risk classes, and workspace-aware native rebuild command hints with working-directory context, gated away from destructive/lifecycle/deploy/network scripts without modifying source, manifests, lockfiles, env files, credentials, IDE settings, workspace metadata, dependencies, build outputs, or executing scripts. Future work should add saved cleanup playbooks and broader per-manager recovery guidance. |
| Xcode review | Expected from developer cleaners and especially important for iOS/macOS developers | Exists/partial | `reclaimer xcode` and app Xcode Review now report DerivedData, module/documentation caches, Products, Archives, DeviceSupport, simulator devices, runtimes, logs, preview simulator data, protected developer-state roots, Xcode/simctl guidance, and local audit records without modifying Xcode files. Future work should add richer version/age gates and Xcode UI deep links where macOS allows it. |
...
| Trash-first cleanup | Expected safety behavior | Exists/partial | Ryddi reports uncertain/user-visible data for Finder Trash review rather than deleting it. `reclaimer trash` and app Trash Review now report current user Trash size, largest items, permission state, Finder guidance, and local audit records without emptying Trash. Future work should add richer Finder Trash integration where macOS allows it. |
| App-managed holding records | Strong differentiator | Exists/partial | Holding metadata, CLI listing, app Holding Area, and manual Finder recovery guidance exist. Future work should add safer identity-bound restore primitives if macOS provides them. |
| Recovery and undo guidance | Expected when cleaners offer destructive actions; often vague in broad suites | Exists/partial | `RecoveryCenter`, `reclaimer recovery`, and app Recovery Center distinguish manual Finder holding-record recovery from Finder Trash review, dry-run/skipped no-ops, and native-tool guidance. Future work should add richer Trash integration where macOS allows it. |
...
| App uninstaller | Common suite/app-cleaner feature | Exists/partial | Apps & Leftovers plus `reclaimer apps uninstall-preview` now separate app-bundle manual-removal evidence from related support-file review. `reclaimer apps uninstall --dry-run` creates evidence only, while `--yes` is rejected because macOS cannot bind a final Trash mutation to the reviewed bundle. Bulk deletion, vendor uninstallers, smart leftover deletion, and automated app-bundle Trash remain future work. |
| Developer cache packs | Ryddi beachhead | Exists/partial | Xcode Review, Package Cache Review, Project Dependencies Review, AI-agent review, Docker/Colima native guidance, and expanded IDE/mobile cache rules exist. Go deeper on version-aware Xcode cleanup, per-project VCS/tool context, JetBrains, VS Code, Android/Flutter, and destructive Docker/Colima confirmation UX. |
| Docker/Colima cleanup | Risky but important for target user | Exists/partial | Native-tool preview receipts describe inspect/prune/reset commands, risk, and non-claims; read-only inventory adds Docker storage buckets, objects, Colima profiles, and command outcomes. Homebrew alone may execute after a fresh same-process preview; Docker/Colima execution remains guidance-only. Future work is deeper confirmation UX for destructive Docker/Colima native prune flows. Never raw-delete VM disks automatically. |
...
| Scheduled maintenance | Expected from automation tools | Exists/partial | `ScheduleConfiguration`, `schedule preview`, and `schedule install` generate XML-safe report-only LaunchAgent plists for Developer/General/All presets, built-in templates, or saved scope sets. Installation creates a new plist only; replacement/removal remains manual Finder work. Only allow unattended cleanup for explicit allowlisted classes in a future release. |
...
4. Recovery Center now covers manual Finder holding-record recovery and receipt-based guidance for Trash, dry-run, skipped, and native-tool actions. Add richer Trash integration where macOS allows it.
...
2. Container rule pack: Docker/Colima inventory, volumes/images/build cache, native prune guidance, VM disk warnings. Preview receipts and read-only live inventory exist; Homebrew alone may execute after a fresh same-process preview, while Docker/Colima native cleanup remains guidance-only.
3. Remote Targets: agentless SSH/VPS evidence reports now extend Ryddi's developer-first cleanup model to Linux servers with safe probes, disk/inode pressure, journald/APT/Docker/deploy-release/large-file buckets, native guidance, saved remote growth diffs, redacted exports, and no remote cleanup execution.
4. Xcode Review now reports DerivedData, ModuleCache, DocumentationCache, Products, DeviceSupport, Archives, simulator logs/devices/runtimes, preview simulator data, and protected developer state without modifying files. Next: richer version/age gates, Xcode Organizer/Settings deep links, and safer user-controlled cleanup workflows after active-handle checks.
5. Package manager review now has a report-only cache inventory for Homebrew, npm, pnpm, Yarn, pip, Cargo, Go, Gradle, Maven, CocoaPods, SwiftPM, and Playwright, with protected config/auth paths and native cleanup guidance. Project Dependencies Review now adds project-local dependency/build artifact inventory for node_modules, virtual environments, Swift/Rust/mobile build outputs, Pods, framework caches, protected project roots, local VCS status, saved per-project policies, detected package-manager/tool/package-name context, workspace/monorepo evidence, bounded/redacted package.json script command previews, script-risk classes, and workspace-aware command hints with working directories. Next: richer tool-installed detection, saved cleanup playbooks, and broader per-manager recovery guidance.
6. IDE/mobile packs: JetBrains, VS Code/Cursor/Windsurf, Android Studio/Gradle, Flutter, Playwright browsers.

### Later Modules

1. Apps & Leftovers depth: selected-app dry-run evidence now exists; app-bundle Trash remains manual Finder work. Next depth is deselection, launch-agent/background-item detail, and stronger orphan ownership evidence before any related-file cleanup.
...
- Finder Trash and holding-area review for safe selections, without automatic mutation.
