...
| Project dependency review | Expected for developer cleaners because project-local `node_modules`, virtualenvs, build outputs, and mobile artifacts can dominate disk usage | Exists/partial | `reclaimer projects` and app Project Dependencies now report project-local dependency/build artifact folders, ecosystem/kind/tool/script/workspace/VCS/policy summaries, protected project roots, saved per-project review/preserve/skip policies, detected package managers, workspace/monorepo evidence, accepted package.json scripts, and native rebuild command hints without modifying source, manifests, lockfiles, env files, credentials, IDE settings, workspace metadata, dependencies, build outputs, or executing scripts. Future work should add deeper project-script safety review and richer workspace-specific cleanup guidance. |
...
4. Package manager review now has a report-only cache inventory for Homebrew, npm, pnpm, Yarn, pip, Cargo, Go, Gradle, Maven, CocoaPods, SwiftPM, and Playwright, with protected config/auth paths and native cleanup guidance. Project Dependencies Review now adds project-local dependency/build artifact inventory for node_modules, virtual environments, Swift/Rust/mobile build outputs, Pods, framework caches, protected project roots, local VCS status, saved per-project policies, detected package-manager/tool context, workspace/monorepo evidence, accepted package.json scripts, and command hints. Next: richer tool-installed detection, per-manager command UX, project-script safety review, and workspace-specific cleanup guidance.
