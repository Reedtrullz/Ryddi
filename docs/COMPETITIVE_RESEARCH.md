...
| Project dependency review | Expected for developer cleaners because project-local `node_modules`, virtualenvs, build outputs, and mobile artifacts can dominate disk usage | Exists/partial | `reclaimer projects` and app Project Dependencies now report project-local dependency/build artifact folders, ecosystem/kind summaries, protected project roots, and native rebuild guidance without modifying source, manifests, lockfiles, env files, credentials, IDE settings, dependencies, or build outputs. Future work should add VCS status, package-manager command shortcuts, and saved per-project policies. |
