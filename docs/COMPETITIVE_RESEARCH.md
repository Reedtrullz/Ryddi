...
4. Package manager review now has a report-only cache inventory for Homebrew, npm, pnpm, Yarn, pip, Cargo, Go, Gradle, Maven, CocoaPods, SwiftPM, and Playwright, with protected config/auth paths and native cleanup guidance. Project Dependencies Review now adds project-local dependency/build artifact inventory for node_modules, virtual environments, Swift/Rust/mobile build outputs, Pods, framework caches, protected project roots, local VCS status, saved per-project policies, detected package-manager/tool context, workspace/monorepo evidence, bounded/redacted package.json script command previews, script-risk classes, and gated command hints. Next: richer tool-installed detection, per-manager command UX, and workspace-specific cleanup guidance.
