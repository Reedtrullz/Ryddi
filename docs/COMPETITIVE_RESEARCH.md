...
| Project dependency review | Expected for developer cleaners because project-local `node_modules`, virtualenvs, build outputs, and mobile artifacts can dominate disk usage | Exists/partial | `reclaimer projects` and app Project Dependencies now report project-local dependency/build artifact folders, ecosystem/kind/VCS/policy summaries, protected project roots, saved per-project review/preserve/skip policies, and native rebuild command hints without modifying source, manifests, lockfiles, env files, credentials, IDE settings, dependencies, or build outputs. Future work should add richer package-manager command detection and project-script awareness. |
