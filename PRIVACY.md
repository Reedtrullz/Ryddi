...
Session Review can reveal a transcript in Finder, move one explicitly confirmed inactive transcript to Finder Trash, or create a private gzip archive under `~/Library/Application Support/Ryddi/Session Archives`. Archive verifies gzip integrity and revalidates the unchanged source before moving the original to Finder Trash. If validation or open-file checks fail, the original remains. Archive files contain the original private transcript content and are readable only by the current user; Ryddi does not upload them.
